...
Compilation options: -a /usr/local/share/faust/max-msp/max-msp64.cpp -lang cpp -i -ct 1 -cn bamodulex -es 1 -omp -mcd 16 -uim -double -ftz 0 -vec -lv 0 -vs 32
------------------------------------------------------------ */

#ifndef  __bamodulex_H__
#define  __bamodulex_H__
...
class bamodulex : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		m->declare("compile_options", "-a /usr/local/share/faust/max-msp/max-msp64.cpp -lang cpp -i -ct 1 -cn bamodulex -es 1 -omp -mcd 16 -uim -double -ftz 0 -vec -lv 0 -vs 32");
		m->declare("filename", "bamodulex.dsp");
		m->declare("name", "bamodulex");
	...
	virtual bamodulex* clone() {
		return new bamodulex();
	}
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("bamodulex");
		ui_interface->closeBox();
	}
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		double fZec0[32];
		#pragma omp parallel\
			firstprivate(input0_ptr, input1_ptr, input2_ptr, input3_ptr, output0_ptr, output1_ptr, output2_ptr, output3_ptr)
		{
			for (int index = 0; index < count; index = index + 32) {
				FAUSTFLOAT* input0 = &input0_ptr[index];
				FAUSTFLOAT* input1 = &input1_ptr[index];
				FAUSTFLOAT* input2 = &input2_ptr[index];
				FAUSTFLOAT* input3 = &input3_ptr[index];
				...
				#pragma omp single
				{
					...
					for (int i = 0; i < vsize; i = i + 1) {
						fZec0[i] = double(input0[i]) + double(input1[i]);
					}
				}
				#pragma omp sections
				{
					#pragma omp section
					{
						...
						/* Compute code */
						for (int i = 0; i < vsize; i = i + 1) {
							output0[i] = FAUSTFLOAT(0.5 * (double(input3[i]) + double(input2[i]) + fZec0[i]));
						}
					}
					#pragma omp section
					{
						...
						/* Compute code */
						for (int i = 0; i < vsize; i = i + 1) {
							output1[i] = FAUSTFLOAT(0.5 * (double(input0[i]) + double(input3[i]) - (double(input1[i]) + double(input2[i]))));
						}
					}
					#pragma omp section
					{
						...
						/* Compute code */
						for (int i = 0; i < vsize; i = i + 1) {
							output2[i] = FAUSTFLOAT(0.5 * (double(input0[i]) + double(input2[i]) - (double(input1[i]) + double(input3[i]))));
						}
					}
					#pragma omp section
					{
						...
						/* Compute code */
						for (int i = 0; i < vsize; i = i + 1) {
							output3[i] = FAUSTFLOAT(0.5 * (fZec0[i] - (double(input2[i]) + double(input3[i]))));
						}
					}
				}
			}
	...
	#define FAUST_FILE_NAME "bamodulex.dsp"
	#define FAUST_CLASS_NAME "bamodulex"
	#define FAUST_COMPILATION_OPIONS "-a /usr/local/share/faust/max-msp/max-msp64.cpp -lang cpp -i -ct 1 -cn bamodulex -es 1 -omp -mcd 16 -uim -double -ftz 0 -vec -lv 0 -vs 32"
 ...
 */
class bamodulex_poly : public dsp_voice_group, public dsp_poly {
    ...
    private:
        ...
        {
            if (val == FAUSTFLOAT(1)) {
                static_cast<bamodulex_poly*>(arg)->allNotesOff(true);
            }
        }
    ...
    public:
         ...
         * Constructor.
         *
         * @param dsp - the dsp to be used for one voice. Beware: bamodulex_poly will use and finally delete the pointer.
        ...
        virtual bamodulex_poly* clone()
        {
            return new bamodulex_poly(fDSP->clone(), int(fVoiceTable.size()), fVoiceControl, fGroupControl);
        }
...
struct dsp_poly_factory : public dsp_factory {
    ...
    {
        dsp_poly* dsp_poly = new bamodulex_poly(adaptDSP(fProcessFactory->createDSPInstance(), is_double), nvoices, control, group);
        if (fEffectFactory) {
...
void faust_allocate(t_faust* x, int nvoices)
{
    ...
    if (nvoices > 0) {
    ...
    #endif
        x->m_dsp = new bamodulex_poly(new bamodulex(), nvoices, true, true);
    #ifdef POLY2
        ...
        x->m_dsp = createSRAdapter<FAUSTFLOAT>(new bamodulex(), DOWN_SAMPLING, UP_SAMPLING, FILTER_TYPE);
    }
...
void* faust_new(t_symbol* s, short ac, t_atom* av)
{
    ...
    bamodulex* tmp_dsp = new bamodulex();
