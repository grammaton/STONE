...
Compilation options: -a /usr/local/share/faust/max-msp/max-msp64.cpp -lang cpp -i -ct 1 -cn mto1ox -es 1 -omp -mcd 16 -uim -double -ftz 0 -vec -lv 0 -vs 32
------------------------------------------------------------ */
...
class mto1ox : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		m->declare("compile_options", "-a /usr/local/share/faust/max-msp/max-msp64.cpp -lang cpp -i -ct 1 -cn mto1ox -es 1 -omp -mcd 16 -uim -double -ftz 0 -vec -lv 0 -vs 32");
		m->declare("filename", "mto1ox.dsp");
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		double fZec0[32];
		#pragma omp parallel\
			firstprivate(input0_ptr, input1_ptr, input2_ptr, output0_ptr, output1_ptr, output2_ptr, output3_ptr)
		{
			for (int index = 0; index < count; index = index + 32) {
				...
				#pragma omp sections
				{
					...
					#pragma omp section
					{
						...
						/* Compute code */
						for (int i = 0; i < vsize; i = i + 1) {
							fZec0[i] = std::cos(double(input2[i]));
						}
					}
				}
				#pragma omp sections
				{
					#pragma omp section
					{
						...
						/* Compute code */
						for (int i = 0; i < vsize; i = i + 1) {
							output2[i] = FAUSTFLOAT(double(input0[i]) * std::sin(double(input2[i])));
						}
					}
					#pragma omp section
					{
						...
						/* Compute code */
						for (int i = 0; i < vsize; i = i + 1) {
							output3[i] = FAUSTFLOAT(double(input0[i]) * fZec0[i] * std::cos(double(input1[i])));
						}
					}
					#pragma omp section
					{
						...
						/* Compute code */
						for (int i = 0; i < vsize; i = i + 1) {
							output1[i] = FAUSTFLOAT(double(input0[i]) * std::sin(double(input1[i])) * fZec0[i]);
						}
					}
				}
			}
	...
	#define FAUST_FILE_NAME "mto1ox.dsp"
	#define FAUST_CLASS_NAME "mto1ox"
	#define FAUST_COMPILATION_OPIONS "-a /usr/local/share/faust/max-msp/max-msp64.cpp -lang cpp -i -ct 1 -cn mto1ox -es 1 -omp -mcd 16 -uim -double -ftz 0 -vec -lv 0 -vs 32"
 ...
 */
class mto1ox_poly : public dsp_voice_group, public dsp_poly {
    ...
    private:
        ...
        {
            if (val == FAUSTFLOAT(1)) {
                static_cast<mto1ox_poly*>(arg)->allNotesOff(true);
            }
        }
    ...
    public:
         ...
         * Constructor.
         *
         * @param dsp - the dsp to be used for one voice. Beware: mto1ox_poly will use and finally delete the pointer.
        ...
        virtual mto1ox_poly* clone()
        {
            return new mto1ox_poly(fDSP->clone(), int(fVoiceTable.size()), fVoiceControl, fGroupControl);
        }
...
struct dsp_poly_factory : public dsp_factory {
    ...
    {
        dsp_poly* dsp_poly = new mto1ox_poly(adaptDSP(fProcessFactory->createDSPInstance(), is_double), nvoices, control, group);
        if (fEffectFactory) {
...
void faust_allocate(t_faust* x, int nvoices)
{
    ...
    if (nvoices > 0) {
    ...
    #endif
        x->m_dsp = new mto1ox_poly(new mto1ox(), nvoices, true, true);
    #ifdef POLY2
        ...
        x->m_dsp = createSRAdapter<FAUSTFLOAT>(new mto1ox(), DOWN_SAMPLING, UP_SAMPLING, FILTER_TYPE);
    }
