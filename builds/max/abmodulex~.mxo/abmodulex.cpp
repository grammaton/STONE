/* ------------------------------------------------------------
name: "abmodulex"
Code generated with Faust 2.68.2 (https://faust.grame.fr)
Compilation options: -a /usr/local/share/faust/max-msp/max-msp64.cpp -lang cpp -i -ct 1 -cn abmodulex -es 1 -mcd 16 -uim -double -ftz 0
------------------------------------------------------------ */

#ifndef  __abmodulex_H__
#define  __abmodulex_H__
...
class abmodulex : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		m->declare("compile_options", "-a /usr/local/share/faust/max-msp/max-msp64.cpp -lang cpp -i -ct 1 -cn abmodulex -es 1 -mcd 16 -uim -double -ftz 0");
		m->declare("filename", "abmodulex.dsp");
		m->declare("name", "abmodulex");
	}
	...
	virtual abmodulex* clone() {
		return new abmodulex();
	}
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("abmodulex");
		ui_interface->closeBox();
	}
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			double fTemp0 = double(input3[i0]);
			double fTemp1 = double(input2[i0]);
			double fTemp2 = double(input0[i0]);
			double fTemp3 = double(input1[i0]);
			double fTemp4 = fTemp2 + fTemp3;
			output0[i0] = FAUSTFLOAT(0.5 * (fTemp0 + fTemp1 + fTemp4));
			output1[i0] = FAUSTFLOAT(0.5 * (fTemp2 + fTemp0 - (fTemp3 + fTemp1)));
			output2[i0] = FAUSTFLOAT(0.5 * (fTemp2 + fTemp1 - (fTemp3 + fTemp0)));
			output3[i0] = FAUSTFLOAT(0.5 * (fTemp4 - (fTemp1 + fTemp0)));
		}
	}
	...
	#define FAUST_FILE_NAME "abmodulex.dsp"
	#define FAUST_CLASS_NAME "abmodulex"
	#define FAUST_COMPILATION_OPIONS "-a /usr/local/share/faust/max-msp/max-msp64.cpp -lang cpp -i -ct 1 -cn abmodulex -es 1 -mcd 16 -uim -double -ftz 0"
 ...
 */
class abmodulex_poly : public dsp_voice_group, public dsp_poly {
    ...
    private:
        ...
        {
            if (val == FAUSTFLOAT(1)) {
                static_cast<abmodulex_poly*>(arg)->allNotesOff(true);
            }
        }
    ...
    public:
         ...
         * Constructor.
         *
         * @param dsp - the dsp to be used for one voice. Beware: abmodulex_poly will use and finally delete the pointer.
        ...
        virtual abmodulex_poly* clone()
        {
            return new abmodulex_poly(fDSP->clone(), int(fVoiceTable.size()), fVoiceControl, fGroupControl);
        }
...
struct dsp_poly_factory : public dsp_factory {
    ...
    {
        dsp_poly* dsp_poly = new abmodulex_poly(adaptDSP(fProcessFactory->createDSPInstance(), is_double), nvoices, control, group);
        if (fEffectFactory) {
...
void faust_allocate(t_faust* x, int nvoices)
{
    ...
    if (nvoices > 0) {
    ...
    #endif
        x->m_dsp = new abmodulex_poly(new abmodulex(), nvoices, true, true);
    #ifdef POLY2
        ...
        x->m_dsp = createSRAdapter<FAUSTFLOAT>(new abmodulex(), DOWN_SAMPLING, UP_SAMPLING, FILTER_TYPE);
    }
...
void* faust_new(t_symbol* s, short ac, t_atom* av)
{
    ...
    abmodulex* tmp_dsp = new abmodulex();
